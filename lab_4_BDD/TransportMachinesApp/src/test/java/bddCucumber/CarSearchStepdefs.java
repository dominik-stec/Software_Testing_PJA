...
public class CarSearchStepdefs {
	...
	// }
	//
	// @Given("^we add first car into database as ([a-zA-Z]+ [a-zA-Z0-9]+
	// [A-Z]{2,3}+ [1-9][0-9]{3}+ [a-z]+ p?e?t?r?o?l?d?i?e?s?e?l?
	// m?a?n?u?a?l?a?u?t?o?m?a?t?i?c? [1-3][0-9]{2} [A-I|K] [a-z]+ [1-9][0-9]+)$")
	// public void we_add_first_car_into_database_as(String carAttr) throws
	// Exception {
	...
	// }
	//
	// @Given("^we add second car into database as ([a-zA-Z]+ [a-zA-Z0-9]+
	// [A-Z]{2,3}+ [1-9][0-9]{3}+ [a-z]+ p?e?t?r?o?l?d?i?e?s?e?l?
	// m?a?n?u?a?l?a?u?t?o?m?a?t?i?c? [1-3][0-9]{2} [A-I|K] [a-z]+ [1-9][0-9]+)$")
	// public void we_add_second_car_into_database_as(String carAttr) throws
	// Exception {
	// carTs = carTserv.parseStringToCar(carAttr);
	// carTs.setId(id++);
	// carTserv.create(carTs);
	// }
	//
	// @Given("^we add third car into database as ([a-zA-Z]+ [a-zA-Z0-9]+
	// [A-Z]{2,3}+ [1-9][0-9]{3}+ [a-z]+ p?e?t?r?o?l?d?i?e?s?e?l?
	// m?a?n?u?a?l?a?u?t?o?m?a?t?i?c? [1-3][0-9]{2} [A-I|K] [a-z]+ [1-9][0-9]+)$")
	// public void we_add_third_car_into_database_as(String carAttr) throws
	// Exception {
	// carTs = carTserv.parseStringToCar(carAttr);
	// carTs.setId(id++);
	// carTserv.create(carTs);
	// }
	//
	// @Given("^we add fourth car into database as ([a-zA-Z]+ [a-zA-Z0-9]+
	// [A-Z]{2,3}+ [1-9][0-9]{3}+ [a-z]+ p?e?t?r?o?l?d?i?e?s?e?l?
	// m?a?n?u?a?l?a?u?t?o?m?a?t?i?c? [1-3][0-9]{2} [A-I|K] [a-z]+ [1-9][0-9]+)$")
	// public void we_add_fourth_car_into_database_as(String carAttr) throws
	// Exception {
	// carTs = carTserv.parseStringToCar(carAttr);
	// carTs.setId(id++);
	// carTserv.create(carTs);
	// }
	//
	// @Given("^we add car number two into to delete list as ([a-zA-Z]+ [a-zA-Z0-9]+
	// [A-Z]{2,3}+ [1-9][0-9]{3}+ [a-z]+ p?e?t?r?o?l?d?i?e?s?e?l?
	// m?a?n?u?a?l?a?u?t?o?m?a?t?i?c? [1-3][0-9]{2} [A-I|K] [a-z]+ [1-9][0-9]+)$")
	// public void we_add_car_number_two_into_to_delete_list_as(String carAttr)
	// throws Exception {
	// carTs = carTserv.parseStringToCar(carAttr);
	// carTs.setId(id++);
	// carTsList.add(carTs);
	// }
	//
	// @Given("^we add car number four into to delete list as ([a-zA-Z]+
	// [a-zA-Z0-9]+ [A-Z]{2,3}+ [1-9][0-9]{3}+ [a-z]+ p?e?t?r?o?l?d?i?e?s?e?l?
	// m?a?n?u?a?l?a?u?t?o?m?a?t?i?c? [1-3][0-9]{2} [A-I|K] [a-z]+ [1-9][0-9]+)$")
	// public void we_add_car_number_four_into_to_delete_list_as(String carAttr)
	// throws Exception {
	// carTs = carTserv.parseStringToCar(carAttr);
	// carTs.setId(id++);
	// carTsList.add(carTs);
	...
	// }
	//
	// @Then("^the database shouldnt contains cars from list but should contains
	// cars ([a-zA-Z]+ [a-zA-Z0-9]+ [A-Z]{2,3}+ [1-9][0-9]{3}+ [a-z]+
	// p?e?t?r?o?l?d?i?e?s?e?l? m?a?n?u?a?l?a?u?t?o?m?a?t?i?c? [1-3][0-9]{2} [A-I|K]
	// [a-z]+ [1-9][0-9]+) ___and___ ([a-zA-Z]+ [a-zA-Z0-9]+ [A-Z]{2,3}+
	// [1-9][0-9]{3}+ [a-z]+ p?e?t?r?o?l?d?i?e?s?e?l? m?a?n?u?a?l?a?u?t?o?m?a?t?i?c?
	// [1-3][0-9]{2} [A-I|K] [a-z]+ [1-9][0-9]+)$")
	// public void
	// the_database_shouldnt_contains_cars_from_list_but_should_contains_cars(String
	// carResults_1, String carResult_2) throws Exception{
	//
	// }
